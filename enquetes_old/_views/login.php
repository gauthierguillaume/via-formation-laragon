...
if(isset($_GET['id'])) {
    ...
    // vérification du titre en fonction de l'ID
    ...
    if($title == $verifTitle) {
        ...
        if(isset($_POST['add_utilisateur'])) {
            ...
            $countEmail = $db->prepare('SELECT COUNT(*) 
            FROM utilisateurs 
            ...
            $selectUtilisateur = $db->prepare('SELECT * FROM utilisateurs WHERE utilisateur_mail = ? ');
            ...
            if(empty($count)) {
                ...
                $addUtilisateur = $db->prepare('INSERT INTO utilisateurs SET utilisateur_mail = ?, id_sujet = ?');
                ...
                // Compter le nombre de questions du sujet
                $countQuestionsSujet = $db->prepare('SELECT * FROM questions WHERE id_sujet = ?');
                $countQuestionsSujet->execute([$id]);
                $countQuestions = $countQuestionsSujet->fetchAll();

                // Compter le nombre de reponses de cette utilisateur sur ce sujet
    ...
    echo "Aucune enquête de satisfaction n'a été sélectionnée.";
}
